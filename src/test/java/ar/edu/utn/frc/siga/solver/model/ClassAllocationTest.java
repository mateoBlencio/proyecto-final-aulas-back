...
class ClassAllocationTest {
    ...
    private static SolverEvent event(String commissionKey, int enrolled) {
        return new SolverEvent("a", commissionKey, enrolled,
    ...
    @Test
    @DisplayName("getOvercrowding con aula asignada delega en SolverRoom.overcrowding")
    void overcrowdingWithClassroom() {
        SolverRoom room = new SolverRoom(1, 10, 100);
    ...
    @Test
    @DisplayName("getUnusedCapacity con aula asignada delega en SolverRoom.undercrowding")
    ...
    void pinnedFactory() {
        SolverRoom room = new SolverRoom(1, 30, 100);
        SolverEvent occupiedEvent = event(null, 0);
