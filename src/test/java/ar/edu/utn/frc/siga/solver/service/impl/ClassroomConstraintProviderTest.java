...
class ClassroomConstraintProviderTest {
    ...
    class MinimizeOvercrowding {
        ...
        @Test
        @DisplayName("sin aula asignada: comportamiento real (distinto del documentado) no penaliza")
        void noRoomAssigned() {
            // FIXME: docs/modulos/solver.md afirma "Sin aula asignada, el excedente es el total de
            // inscriptos" (y ClassAllocation.getOvercrowding() tiene una rama explícita `classroom ==
            // null → return event.enrolled()` para eso), pero verificado con ConstraintVerifier la
            // penalización real es 0, no enrolled*100_000. Motivo: `classroom` es un
            // @PlanningVariable no-nullable; mientras esté sin inicializar, Timefold excluye la
            // entidad de forEach(ClassAllocation.class) en el stream de constraints, así que esa
            // rama de getOvercrowding() nunca se ejecuta durante el scoring real (queda alcanzable
            // solo si algo llama el getter fuera del ConstraintProvider). Si un evento queda
            // permanentemente sin aula candidata (value range vacío), el score no refleja ninguna
            // sobreocupación por ese evento: parece feasible sin serlo.
