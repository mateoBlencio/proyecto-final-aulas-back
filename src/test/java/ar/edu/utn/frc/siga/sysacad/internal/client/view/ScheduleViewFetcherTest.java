...
class ScheduleViewFetcherTest {
    ...
    @Test
    @DisplayName("fetch: consulta HorariosComisionesCupos ordenada por Curso, sin tope de filas")
    void fetchQueriesFullViewWithoutRowCap() {
        ...
        when(client.fetchRows(eq("HorariosComisionesCupos"), any(ViewQuery.class), any()))
        ...
        verify(client).fetchRows(eq("HorariosComisionesCupos"), queryCaptor.capture(), any());
        ViewQuery query = queryCaptor.getValue();
        assertThat(query.sort()).isEqualTo("Curso");
