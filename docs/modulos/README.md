...
> Cobertura unitaria real: `AllocationServiceImplTest`,
...
> **`academic`, `space`, `common` y `excelimport` siguen en cero tests**, y
> no hay ningún test de integración (Testcontainers) todavía.

Consecuencias concretas:

- **El comando de ejemplo `./mvnw test -Dtest=SolverServiceImplTest` apunta a una clase
  que no existe.** Es aspiracional, no real
  (el módulo `solver` solo tiene `CaffeinePreviewStoreTest`).
- La infraestructura de Testcontainers (`jdbc:tc:postgresql:16-alpine`) documentada
  no tiene ningún test de integración que la use.
...
Prioridad de cobertura sugerida (mayor ROI primero):
